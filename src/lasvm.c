...
int
lasvm_get_alpha(lasvm_t *self, float *alpha)
{
  int i;
  int l = self->l;
  for (i=0; i<l; i++)
...
int
lasvm_get_g(lasvm_t *self, float *g)
{
  int i;
  int l = self->l;
  for (i=0; i<l; i++)
...
static int
gs1( lasvm_t *self, int i, float epsgr)
{
  int l = self->s;
  real_t g;
...
static int
gs2( lasvm_t *self, int imin, int imax, float epsgr)
{
  int l = self->s;
  real_t gmin, gmax;
  real_t step, ostep, curv;
...
int 
lasvm_process( lasvm_t *self, int xi, float y )
{
  int l = self->l;
  int *i2r = 0;
  float *row = 0;
...
int 
lasvm_reprocess(lasvm_t *self, float epsgr)
{
  int status;
  if (self->s != self->l)
    lasvm_error("lasvm_process(): internal error\n");
...
int 
lasvm_finish(lasvm_t *self, float epsgr)
{
  int iter = 0;
  int siter = 0;
  int status = self->l;
